...
def main():
    ...
    # First, let's back up the questions directory
    import shutil
    ...
    run_git(["commit", "-m", "feat: seed 2023 questions for Physics, Chemistry, and Mathematics"])
    ...
    run_git(["commit", "-m", "feat: seed 2024 questions for Physics, Chemistry, and Mathematics"])
    ...
    run_git(["commit", "-m", "feat: seed 2025 questions for Physics, Chemistry, and Mathematics"])
    ...
    # Let's modify a file in questions/2023/physics/mechanics/hard_JEE-2023-PHY-MEC-001.json
    # We'll fix a small detail (e.g. increase marks or update explanation text)
    file_to_fix = "questions/2023/physics/mechanics/hard_JEE-2023-PHY-MEC-001.json"
    if os.path.exists(file_to_fix):
        with open(file_to_fix, "r") as f:
            data = json.load(f)
        data["explanation"] += " (Note: verified via rotational mechanics standard test cases)."
    ...
    print("Git branching model set up successfully!")
